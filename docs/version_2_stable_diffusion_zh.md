...
第二版加入 Stable Diffusion 图生图原型，用大模型把源图像转译成新古典主义油画感。

和第一版不同，这一版不再追求实时逐帧处理。客户确认不需要实时生成，所以交互方式改成“输入一张图”或“从摄像头抓取当前一帧”，再等待模型生成结果。这样更符合 Stable Diffusion 的实际速度，也更容易得到质量稳定的图片。

默认 prompt 已经改成英文风格迁移提示词，重点强调保留原图结构、透视、镜头角度、物体位置和构图，只改变视觉风格。这样比直接要求模型“生成一张新的新古典主义图像”更符合项目目标。

这一版先不训练，重点是先验证：

1. 摄像头画面能不能作为输入
2. Stable Diffusion 图生图能不能把源图像推向新古典主义风格
